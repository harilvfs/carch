...
def install_chaotic_aur():
    ...
    if (
        ...
        print_error(
            "Failed to fetch the Chaotic AUR key. Check your internet connection."
        )
        sys.exit(1)
    ...
    if (
        run(
            ["sudo", "pacman-key", "--lsign-key", "3056513887B78AEB"], check=False
        ).returncode
        != 0
    ):
    ...
    backup_path = os.path.join(
        BACKUP_DIR, f"pacman.conf.bak.{random.randint(10000, 99999)}"
    )
    ...
    if (
        run(
            ["sudo", "tee", "-a", PACMAN_CONF], input=entry.encode(), check=False
        ).returncode
        != 0
    ):
