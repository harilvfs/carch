...
#[test]
fn search_history_records_submitted_queries() {
    let mut fx = fixture();
    fx.app.handle_key_normal_mode(key(KeyCode::Char('/')));
    for ch in "git".chars() {
        fx.app.handle_search_input(key(KeyCode::Char(ch)));
...
#[test]
fn search_history_is_capped_at_five() {
    let mut fx = fixture();
    for n in 0..8 {
        let q = format!("q{n}");
        fx.app.handle_key_normal_mode(key(KeyCode::Char('/')));
...
#[test]
fn typing_a_char_resets_history_browsing() {
    let mut fx = fixture();
    fx.app.handle_key_normal_mode(key(KeyCode::Char('/')));
    for ch in "git".chars() {
        fx.app.handle_search_input(key(KeyCode::Char(ch)));
    ...
    fx.app.handle_search_input(key(KeyCode::Up));
    assert_eq!(fx.app.search.history_idx, Some(0));
    fx.app.handle_search_input(key(KeyCode::Char('x')));
    assert_eq!(fx.app.search.history_idx, None);
    assert_eq!(fx.app.search.input, "gitx");
}
