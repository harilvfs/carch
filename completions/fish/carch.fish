function __fish_carch_no_subcommand
    for i in (commandline -opc)
        switch $i
            case --help -h --version -v --log --no-cleanup check-update update uninstall
                return 1
        end
    end
    return 0
...
complete -c carch -n '__fish_carch_no_subcommand' -l no-cleanup -d "Disable cache cleanup"
complete -c carch -n '__fish_carch_no_subcommand' -a 'check-update' -d "Check if a new version of Carch is available"
complete -c carch -n '__fish_carch_no_subcommand' -a 'update' -d "Interactively update Carch"
complete -c carch -n '__fish_carch_no_subcommand' -a 'uninstall' -d "Interactively uninstall Carch"