...
install_font() {
	...
	wget -q -O "$font_download_path" "$download_url" || {
		echo -e "${RED}Failed to download $font_name.${NC}"
		exit 1
	}

	echo -e "${CYAN}:: Unzipping $font_name...${NC}"
	unzip -q "$font_download_path" -d "$font_extract_dir" || {
		echo -e "${RED}Failed to unzip $font_name.${NC}"
		exit 1
	}
	...
	rm -rf "$download_dir" "$font_extract_dir" "$font_download_path"
