...
impl App {
    ...
    pub fn load_scripts(&mut self, modules_dir: &Path) -> io::Result<()> {
        ...
        if !self.visible_items.is_empty() {
            let default_select = self.visible_items.iter()
                .find(|&&idx| idx < self.scripts.items.len() && !self.scripts.items[idx].is_category_header)
                .copied();
    ...
    pub fn update_visible_items(&mut self) {
        ...
        if let Some(selected) = current_selected {
            if !self.visible_items.contains(&selected) && !self.visible_items.is_empty() {
                ...
                let category_header = self.visible_items.iter()
                    .find(|&&idx| {
            ...
            *expanded = !*expanded;
            ...
            if ui_options.log_mode {
                let _ = crate::commands::log_message(
                    "INFO", 
                    &format!("Category '{}' {}", category, if *expanded { "expanded" } else { "collapsed" })
                );
            }
    ...
    pub fn handle_search_input(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Right => {
                ...
                if self.search_cursor_position == self.search_input.len() && self.autocomplete_text.is_some() {
...
pub fn run_ui_with_options<F>(modules_dir: &Path, run_script_callback: F, options: UiOptions) -> io::Result<()>
where
    F: Fn(&Path) -> io::Result<()>,
{
    if options.log_mode {
    ...
    if options.log_mode {
        let _ = crate::commands::log_message("INFO", &format!("Loaded {} scripts in {} categories", 
                ...
                match event {
                    Event::Key(key) => {
                        if options.log_mode {
                            ...
                            let _ = crate::commands::log_message("DEBUG", &format!("Key pressed: {} in mode: {:?}", key_name, app.mode));
                        }
                        ...
                        match app.mode {
                            AppMode::Normal => {
                                if key.code == KeyCode::Char('p') && !options.show_preview {
                                    if options.log_mode {
                                        let _ = crate::commands::log_message("INFO", "Preview toggle attempted but previews are disabled");
                                    }
                                } else {
                            ...
                            AppMode::Preview => {
                                if !options.show_preview {
                                    app.mode = AppMode::Normal;
                                    if options.log_mode {
                                        let _ = crate::commands::log_message("INFO", "Exiting preview mode because previews are disabled");
                                    }
                                } else {
                                ...
                                if let Some(selected) = app.scripts.state.selected() {
                                    if selected < app.scripts.items.len() && !app.scripts.items[selected].is_category_header {
                                        if let Some(script_path) = app.get_script_path() {
                                            if options.log_mode {
                                                let script_name = script_path.file_name().unwrap_or_default().to_string_lossy();
                                                let _ = crate::commands::log_message("INFO", &format!("Selected script for execution: {}", script_name));
                                            }
                                            ...
                                            if options.log_mode {
                                                let _ = crate::commands::log_message("INFO", "Exiting UI to run script");
                                            }
                                            
                                            run_script_callback(&script_path)?;
                                            
                                            if options.log_mode {
                                                let _ = crate::commands::log_message("INFO", "Script execution completed, returning to UI");
                                            }
                    ...
                    Event::Mouse(mouse_event) => {
                        if options.log_mode {
                            let _ = crate::commands::log_message("DEBUG", &format!("Mouse event: {:?}", mouse_event));
                        }
...
fn render_preview<B: Backend>(f: &mut Frame<B>, app: &App, area: Rect) {
    let selected_script = app.scripts.state.selected().and_then(|i| {
        app.scripts.items.get(i).map(|s| s.full_name())
    });
