...
function hasOnlyChild(node, kidFn) {
  return node.alternatives.length === 1 && node.alternatives[0].elements.length === 1 && (!kidFn || kidFn(node.alternatives[0].elements[0]));
}
...
function isZeroLengthNode(node) {
  return node.type === AstTypes.Assertion || node.type === AstTypes.Directive || node.type === AstTypes.Quantifier && !node.min;
}
...
function parseFragment(pattern2, options) {
...
function toDetails(pattern2, options) {
  ...
  pattern2 = possessive(recursion(generated.pattern));
  const atomized = atomic(pattern2, { useEmulationGroups: !opts.avoidSubclass });
  const useEmulationGroups = atomized !== pattern2 && !opts.avoidSubclass;
  pattern2 = atomized;
  const result = {
    pattern: pattern2,
...
var EmulatedRegExp = class _EmulatedRegExp extends RegExpSubclass {
  #strategy;
  constructor(pattern2, flags, options) {
    const opts = {
      useEmulationGroups: false,
      strategy: null,
      ...options
    };
    super(pattern2, flags, { useEmulationGroups: opts.useEmulationGroups });
    if (opts.strategy) {
