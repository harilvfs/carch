...
use carch_core::{ego_tree::NodeId, Command, Config, ConfigValues, ListNode, TabList};
use ratatui::{
...
impl AppState {
    pub fn new(args: Args) -> Self {
        let tabs = carch_core::get_tabs(!args.override_validation);
        ...
        let longest_tab_display_len = tabs
            ...
            .max()
            .unwrap_or(22) as u16; // 22 is the length of  title
    ...
    pub fn get_keybinds(&self) -> (&str, Box<[Shortcut]>) {
        match self.focus {
            ...
            Focus::List => {
                ...
                hints.push(Shortcut::new("Exit carch", ["q", "CTRL-c"]));
            ...
            Focus::TabList => (
                "Tab list",
                Box::new([
                    Shortcut::new("Exit carch", ["q", "CTRL-c"]),
